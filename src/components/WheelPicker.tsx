...
const WheelPicker: React.FC<WheelPickerProps & RefAttributes<FlatList<any>>> = forwardRef((props, ref) => {
  ...
  const offsets = useMemo(() => [...Array(wheelData.length)].map((x, i) => i * itemHeight), [wheelData, itemHeight]);

  const scrollHandler = useAnimatedScrollHandler((event: { contentOffset: { y: number } }) => {
  ...
  const WheelItem = memo(({ item, index }: { item: any; index: number }) => {
    ...
    return (
      <Animated.View style={animatedStyle}>
        <Text key={index} style={[styles.label, { height: itemHeight, color: textColor }]}>
          {item?.label}
        </Text>
      </Animated.View>
    );
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={[styles.animatedContainer, { height: containerHeight }]}>
          <AnimatedFlatList
            ...
            data={wheelData}
            renderItem={({ item, index }: { item: any; index: any }) => <WheelItem index={index - visibleRest} item={item} />}
